...
```{math}
:label: A1
\mathrm{nutrient~ uptake~rate} =  C \frac{R}{R+K_i}
```
- In Equation [#A1], the constant of proportionality $C$ is a linear function of the quota, $Q_i$,
```{math}
:label: A2
C = V^{hi}_{max.i} - \left(V^{hi}_{max.i} - V^{lo}_{max.i}  \right) \frac{Q_i-Q_{min,i}}{Q_{max,i}-Q_{min,i}}
```

...
During this extended larval duration, larvae face predators, pathogens, trranport away from favorable habitats, and other risks.
...
1. *The echinoid mode of cloning*
	In the cloning mode most commonly observed in echinoids [sea urchins](wiki:Sea_urchin), larvae bifurcate during development, partway into their larval duration and prior to having grown and developed enough to metamorphose.
	...
	Whether it redifferentiates or regrows lost parts, each clone has to grow back to a size at which it is capable of metamorphosis (or of cloning again). The model neglects this difference in developmental mode of clones: Consistent with using size as a proxy, the model treats all larvae of size $s$ the same. 

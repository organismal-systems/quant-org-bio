...
The rationale for assuming that standing stock is roughly proportional to losses to higher trophic levels rests on their density-independent "background" mortality term[^mort]

[^mort]: That term is $\mu N_i$, where $N_i$ is the number of cells of size class $i$ and $\mu$ is the mortality rate. See [this explanation of the model](./Litchman_etal2009.md) and the original paper for details.

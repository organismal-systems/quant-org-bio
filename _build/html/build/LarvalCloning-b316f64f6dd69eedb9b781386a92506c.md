...
Recently, it has become clear that the larvae of a number of marine invertebrates can [clone](wiki:Cloning), either intrinsically or in response to environmental cues such as chemical stressors or predators.
...
If cloning itself is not metabolically costly, then a reasonable baseline approximation is that **the sizes of two daughters produced by cloning add up approximately to the size of the original larva.**

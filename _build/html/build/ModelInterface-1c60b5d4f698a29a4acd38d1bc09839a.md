...
5. Scroll down to the section labeled **Setting mixing event intervals, and running the simulation**.
   These textboxes enable you to set the number of days between mixing events, the number of mixing intervals to run the simulation, and how many times to record data as the cell numbers, nutrient quotas and nutrient concentration change between mixing events
...
6. **Examine the textual and graphical output.**
   ...
   *Note that the vertical axis uses a **log scale**. That means one major grid tick represents a factor of 10 in cell numbers.*[^num]
...
For this model, a motivating question can be something like, *Does mixed layer depth have an effect on diatom cell numbers?*
...
A hypothesis for this model might be something like, *Mixing interval determines the size of the dominant diatoms.*
...
One of the best ways to quickly assess a hypotheses is to make a plot that visually shows the presence or absence of a trend.
We are familiar with this approach for empirical data.
The same approach is informative with simulation results.
...
### Have a plan
Changing parameters haphazardly can be a useful way to gain familiarity with model inputs and outputs, but to gain scientific insights it's best to execute a planned series of runs, in which you change a small number of parameters to test a hypothesis. understand cause and effect 

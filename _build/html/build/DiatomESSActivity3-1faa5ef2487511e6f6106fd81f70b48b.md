...
5. In the **Setting mixing event intervals, and running the simulation** section, 
   ...
   This relatively large number of mixing intervals will be helpful in determining the long-term outcomes of diatom population dynamics.

These parameters will apply to all the model runs in this Activity.

## Part 1: Is a diatom variant an ESS?
Because testing all size classes would take too long, in this part of the Activity we will only test three: the smallest, largest and middle sizes $i=0,7,15$ (that is, $s=2.5,7.17,12.5$).

...
The advantages of running notebooks locally include:
- Faster execution, depending on the speed of the local computer
- No time-out for idle session (Binder suspends a session if it has been idle too long)
- Editing notebooks or codes (you can edit on Binder, but those are temporarly copies that do not persist past the end of that Binder session)

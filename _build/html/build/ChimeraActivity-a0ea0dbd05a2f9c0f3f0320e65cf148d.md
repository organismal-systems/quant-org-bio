...
- The model is executed in three stages:
  - The relationships between "organism-based" morphological parameters (*e.g.*, the amount of tissue, general shape, *etc*.) and geometrical characteristics (*e.g.*, diameter, length, *etc*.) are codified in [](./ChimeraGeom.ipynb).
  - The geometrical and fluid dynamical characteristics are calculated and saved in a "morphology" file, in [](./ChimeraDesign.ipynb)

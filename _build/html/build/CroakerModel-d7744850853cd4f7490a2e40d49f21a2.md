...
That is because sperm are usually available in much greater numbers, most of which never encounter an egg and hence play no part in reproduction.  
    ...
    So, we now make the assumption that we are modeling only females, and adjust populations, fecundity etc. accordingly (for example, if half the eggs are of each gender, the effective fecundity in the female-only model is 50% of the total observed fecundity).

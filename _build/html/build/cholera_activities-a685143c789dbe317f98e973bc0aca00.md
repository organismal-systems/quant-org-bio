...
The activities below use the model of cholera by Koelle *et al*. (2005) as a tool with which to get some quantitative insights into how much alternative hypothetial intervention strategies might benefit an affected population.

## Sensitivity to parameters
To base an assessment of mitigation strategies on a model, we need to understand the model's basic characteristics. In particular, we need to know how different parameters affect key statistics like the average number of cholera cases, the average transmission rate, the fraction of a population that is likely to susceptible to contracting cholera, etc. Below is a table of model parameters. The middle column has default values; the right column has estimates for these parameters by Koelle et al. (2005).
...
:::
Given possible inaccuracies of data, simplifying assumptions in the model,  uncertainties about parameter-fitting procedures, etc., Koelle et al.'s parameters might be slightly or even wildly erroneous. Let's get a sense for how sensitive the model is to some parameters. 
(a) Koelle et al. suggest the seasonal fluctuations correspond to roughly c=1 in the cholera worksheet. How does changing this parameter from the default (c=0) qualitatively change the temporal patterns of infected and susceptible people in the time series plots? How does it affect the average monthly cases and population fraction of susceptible?
(b) Koelle et al. suggest the climate fluctuations correspond to roughly b=10 in the cholera worksheet. How does changing this parameter from the default (b=1.5) qualitatively change the temporal patterns of infected and susceptible people in the time series plots? How does it affect the average monthly cases and population fraction of susceptible?
(c) Koelle et al. suggest the immune period is on the order of ten years (120 months). How does this parameter change affect the temporal patterns and summary statistics? 
(d) Koelle et al. suggest the noise parameter ε is 0.95. How does this parameter change affect the temporal patterns and summary statistics? 

\section{📖️ Transport of organisms by currents and turbulence}

Most environmental fluids -- masses of air in weather systems, or the water in lakes, rivers, estuaries, and the open ocean -- are moving at velocities that can vary from almost undetectable to extremely rapid.
...
\subsection{Examples of transport in environmental flows}

To get a concrete sense of the relevant phenomena, let's begin with a couple examples:

\subsubsection{Reproductive biology}

Most benthic marine invertebrates are \href{https://en.wikipedia.org/wiki/Spawning}{free spawning}, meaning that they release their gametes into the water.
Sperm must encounter eggs for \href{https://en.wikipedia.org/wiki/Fertilisation}{fertilization} to happen, but this is more challenging than it might seem.
That's because the surrounding water is almost always moving, due to waves, currents and tides.
Movement of the water into which gametes are released means that in a very short time -- possibly within a few seconds -- these gametes have been moved far from the adults that released them.
...
Given synchronized spawning, successful reproduction then depends on maximizing encounters of eggs with approriate concentrations of sperm, sufficient to make fertilization successful but not so dense as to make \href{https://en.wikipedia.org/wiki/Polyspermy}{polyspermy} likely.
The encounter rate of eggs with sperm -- and hence, reproductive success -- is determined in large part by how fast these gametes sink, rise or swim, along with the rates at which they are transported, mixed and diluted by environmental flows.
...
If we know the density of the pesticide mixture, we can calculate the sinking velocity of the liquid pesticide droplets using the calculators in \href{./rs1.ipynb}{this worksheet}.
...
\subsection{Transport by advection}

Transport of organisms and other immersed objects or substances by moving fluid is called \href{https://en.wikipedia.org/wiki/Advection}{advection}\footnote{The phenomenon of population, mass, momentum, energy or any other \textit{conserved quantity} being carried by moving fluid is called \textit{advection} or \textit{convection}.
...
Environmental flows often \textit{advect} organisms, particles and solutes much faster and in different directions than they would otherwise move.
For example, many organisms move vertically -- they sink or rise under the influence of gravity and buoyancy.
...
\subsection{Mixing by turbulence}

Most environmental flows also have some level of \href{https://en.wikipedia.org/wiki/Turbulence}{turbulence}.
If you have been aboard an airplane flying through turbulent air, you have felt first-hand that turbulence is composed of \href{https://en.wikipedia.org/wiki/Eddy\_(fluid\_dynamics)}{eddies} and other small-scale but energetic fluctuations of fluid velocity.
Turbulence is also a very effective mechanism for mixing; this is frequently called \href{https://en.wikipedia.org/wiki/Turbulent\_diffusion}{turbulent diffusion}, because it in some ways mimics the effects of molecular diffusion but is much faster.
...
This will give you a sense for how much turbulence has sped up the mixing process\footnote{Even this is an overestimate of mixing without turbulence, because temperature differences as the coffee cools cause some mixing currents!}.
...
\subsection{Distributions of organisms in environmental flows}
...
\begin{enumerate}
\item The particles' own velocity relative to the fluid in which they are immersed, such as sinking or rising under the influence of gravity, buoyancy and drag forces.
\item Advection by the directional velocity of the fluid, such as wind or current, often primarily in horizonatal directions.
\item Turbulent eddies, that cause small-scale random movements of individual particles while mixing and spreading a population of particles.
\end{enumerate}

To understand the combined effects of vertical movements with advection and turbulence, it's important to keep in mind the different effects on \textit{population distributions} of advection by currents \textit{vs.} mixing by turbulence.

\begin{itemize}
\item Starting with a group of organisms or a blob of solute, \textit{advection} (with no turbulence) moves the overall average position (in engineering jargon, the center of mass) of that group or blob, such that it  moves with the current over time.
\item Starting with a group of organisms or a blob of solute, \textit{turbulent diffusion} (with no advection) spreads out the group or blob in a similar way to molecular diffusion or random swimming, but much faster except at very small length scales (much smaller than a millimeter).
\end{itemize}
...
The \href{/rs2exec}{next worksheet} gives us tools for quantitatively assessing the consequences of these different scenarios for different types of particles.
...
2. **Individuals die**  
     [](doi:10.1139/cjfas-57-10-2010) assumed that the probability of an adult dying in a given year is a constant, $\mu_{adult}$, for all age classes older than larvae and juveniles ($i=3, 4, \dots$, 7).
    ...
    $$
    for all the adult Year Classes, $i = 3, 4, \dots$, 7.
...
%$$
%p_1(t+1) = F_1 p_1(i) + F_2 p_2(i) + F_3 p_3(i) + F_4 p_4(i) + F_5 p_5(i) + F_6 p_6(i) + F_7 p_7(i)
%$$
    $$
    p_1(t+1) = \sum_{i=1}^7 F_i p_i(t)
    $$
    In words, this equations says that the number of eggs is equal to the number of females in Year Class $i$ multiplied by the number of eggs produced per female ($F_i$), summed across all the Year Classes $i=1,2,\dots, 7$.

...
In this Activity, you will use the size-specific standing stock as a **proxy** that indicates the relative effects of mixed layer characteristics &ndash; mediated by their impacts on diatom population dynamics &ndash; on primary production, prey availability to diatom grazers and intensity of competition among primary producers.
The specific rationale rests on their density-independent "background" mortality term ($\mu N_i$, where $N_i$ is the number of cells of size class $i$ and $\mu$ is the mortality rate).

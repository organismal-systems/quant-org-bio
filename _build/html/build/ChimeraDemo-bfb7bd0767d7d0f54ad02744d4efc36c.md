...
To gain familiarity with the [ChimeraSwim](./ChimeraSwim.ipynb) model, we can start by implementing parameters inspired by observations of the early-stage larvae of a specific taxon, the sand dollar *Dendraster excentricus*.
Many of these values are poorly constrained, having been measured infrequently or not at all.
For these, we will make educated guesses.
...
The parameter values below are illustrative, based loosely on data in two primary references[^ref1)[^ref2].

1. Open this notebook on Binder or on your own computer.
2. Run all cells to initialize the textboxes and Python classes.
3. Define organism-based morphological parameters:  
    - $\rho_{excess}=300$  

...
That is, once established, an initial ESS population of variants can exclude "invasion"[^inv] by a small number of individuals from any other variant.

[^inv]: In ESS's, "invasion" means that an initially small population of a new variant can grow, in the environment created by the established variant.
...
## Multiple ESS's
It's important to keep in mind the key distinction between an ESS and the outcome of competition among variants:
> $\rightarrow$ In an ESS, the "established" variant is allowed to approach its steady state population, and to shape the environment.  
  $\rightarrow$ In competition among variants, the environment may be shaped by a variety of internal and external conditions.

In the diatom example, "shaping the environment" means determining the mixed layer nutrient concentrations.
...
This distinction between ESS's and the outcome of competition has two important implications:
1. The outcome of competition may be, but is not always, an ESS.
2. There may be multiple ESS variants under the same external conditions, each of which once established can exclude all other variants.
...
[^better]: Note that "better" in this context is defined as the successful competitor. For example, it could be that species A has faster uptake of high-concentration nutrients but species B has faster uptake of low-concentration nutrients. If the nutrient is limiting, it will be driven down to low concentrations, so species B is "better" in competitive terms, even though under other circumstances species A might have higher growth. For example, under predation, both populations might be reduced so that nutrients are not limiting, and species A might predominate.

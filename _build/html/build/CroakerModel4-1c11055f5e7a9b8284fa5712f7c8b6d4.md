...
The matrix $\mathbf{A}$ contains the terms of the age-structured model that specified how the croaker population transitions to year $t+1$:
...
This matrix is in a common form in population modeling, called a **Leslie matrix**, in which non-zero terms appear only in the top row and the subdiagonal[^sbd].

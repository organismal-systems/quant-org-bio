...
2. Does it make intuitive sense?  
	...
	- increasing habitat size favors population persistence, because death due to encountering non-habitat is slower[^len]
	- increasing movement favors population extinction, because death due to encountering non-habitat is faster

[^len]: It's worth noting that, if individuals always moved in the same direction, the time to encounter non-habitat would be proportional to $L$. Because individuals move in random directions, the time to encounter non-habitat must increase faster than the same-direction case, and $L^2$ is the next available power of $L$.

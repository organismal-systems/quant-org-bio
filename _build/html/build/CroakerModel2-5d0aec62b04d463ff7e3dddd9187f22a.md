...
That is, a fraction $S_{egg}$ survive from spawning to the end of the Egg larva stage.
...
where $\mu_{YSL}$ is the daily mortality rate during the egg stage and $T_{YSL}$ is the time (in days) a Yolk-Sac Larva requires to develop into the Ocean Larval stage, and so forth for all the larval stages.

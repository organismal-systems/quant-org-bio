...
This matrix is in a common form in population modeling, called a **Leslie matrix**, in which non-zero terms appear only in the top row and the subdiagonal (the set of terms stretching diagonally downwards and to the right, starting from the first entry in the second row).

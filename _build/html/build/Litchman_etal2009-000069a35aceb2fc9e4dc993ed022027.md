...
```
- In Equation [#A1], the constant of proportionality $C_1$ for a cell of size $s_i$ is a linear function of the quota, $Q_i$, expressed relative to the cell's minimum quota, $Q_{min,i}$ and maximum quota, $Q_{max,i}.
```{math}
:label: A2
C_1 = C_2 \frac{Q_i-Q_{min,i}}{Q_{max,i}-Q_{min,i}}
```

...
2. The maximum nutrient uptake rate, $V^{hi}_{max.i}$, increases with diatom size ($b=0.67$) but the amount of nutrient required to attain maximum quota increases even faster ($b=0.81$).

	That means larger cells take longer to fill their quotas than smaller cells.
3. Both the minimum quota, $Q_{min}$, and the maximum quota, $Q_{max}$, increase with diatom size.

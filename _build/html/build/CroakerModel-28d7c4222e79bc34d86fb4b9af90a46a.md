...
That is because sperm are usually available in much greater numbers, most of which never encounter an egg and hence play no part in reproduction.  
    ...
    $$
    In words, this equations says that the number of eggs in year $t+1$ equals the number of females in Year Class $i$ in year $t$ multiplied by the number of eggs produced per female ($F_i$), summed across all the Year Classes $i=1,2,\dots, 7$.

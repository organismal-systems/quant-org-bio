...
3. The model has three types of agents &ndash; grass, sheep and wolves &ndash; that follow these rules:
	...
	- Sheep, gain energy from eating grass if they land on it, consume energy while moving, and are eaten if encountered by wolves.
	Sheep reproduce with a constant user-set probability each time step.
	- Wolves, gain energy from eating sheep if they land on one, consume energy while moving, and reproduce with a constant user-set probability each time step.

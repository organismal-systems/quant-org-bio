...
The KISS[^kiss] model is expressed as a [Partial Differential Equation (PDE)](../../../content/appendix/MathSum/pdes.md) for the changes of a population, $p^*(t^*,x^*)$,
$$
\label{kiss1}
...
[^in]: The notation $\in[a,b]$ means "within the range between $a$ and $b$. 

In Equation [](#kiss1), $^*$ implies dimensional variables:
- $p^*$ has units of $individuals$
- $t^*$ has units of $time$
- $x^T$ has units of $length$
...
This analysis will show that all KISS models are **scale models** of each other, and all are scale models of a.
That is, a KISS model with any choices of $\alpha$, $D$ and for any 

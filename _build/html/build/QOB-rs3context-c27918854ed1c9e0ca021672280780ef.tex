\section{📖️ Mass flux in and out of moving organisms}

We are all familiar with the phenomenon of ``wind chill'' -- we get colder on a chilly, windy day than on a day that is equally cold but has no wind.
...
Just as we used a \href{/rsscaling}{scale model} approach to estimate how fast particles move in fluids, we can use this approach to estimate how molecular transport mechanisms like diffusion are enhanced by particle movement.
The most convenient way to perform this calculation is with another \textit{dimensionless ratio}.
...
\textbf{1. $\mathcal{Sh}$ is a function of velocity (in this case, the sinking speed).}
...
Because Sh depends on velocity, it is effectively summarized as a function of $\mathcal{Re}$ (just as was the \href{/rslookup}{coefficient of drag, $C_d$)}.
Here, $\mathcal{Re}$ summarizes how fluid flows around the particle.

\textbf{2. $\mathcal{Sh}$ is a function of the diffusion rate.}
...
The convenient way to express this is as yet another dimensionless ratio, the \href{https://en.wikipedia.org/wiki/Schmidt\_number}{Schmidt number}, $\mathcal{Sc}$.
...
\textbf{3. For many simple shapes, we have a formula for the mass tranport due to diffusion alone.}
...
\begin{itemize}
\item $d$ is the particle diameter;
\item $C_w$ is the solute concentration in the fluid far from the particle; and,
\item $C_s$ is the concentration at the surface of the particle.
\end{itemize}
...
Then, we can calculate the actual mass flux (including diffusion \textit{and} advection) into a moving particle, $Q_{\mathit{adv}}$.

A worksheet that makes it easy to use Sherwood numbers to calculate the fluxes in and out of spherical organisms or other particles is \href{/rs3}{here}.
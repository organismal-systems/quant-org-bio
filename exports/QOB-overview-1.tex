\chapter{🏞️ Why use models in Organismal Biology?}
...
One reason thinking about and using models can benefit Organismal Biologists is that much of what we do already \textit{implicitly} assumes some type of modeling framework.
For example, all statistics are based on underlying models about probability distributions and random processes that emerge from them (independent events, normal distributions, \textit{etc}.).
Nearly all measurements of environmental characteristics (temperature, pH, salinity, \textit{etc}.) are based on electrical measurements as proxies for the quantity being measured.
...
The best modeling of all is \textit{modeling while experimenting}, when moving quickly between the computer and the laboratory or field site makes the connections between nature and models especially thought-provoking.
...
\textit{\textbf{Models are tools for inferring the logical consequences of specific assumptions.}}
...
\begin{itemize}
\item Formulating a mathematical model requires \textit{specificity} in details that are easy to overlook in conceptual thinking:
Exactly \textit{how} is a process like morphogenesis, synchronized spawning, predator avoidance or gene flow regulated?
...
\item Different scientists, especially scientists from different disciplines, use the \textit{same word with very different meanings}.
...
\textit{\textbf{Models are tools for inferring what we want to know from what we can measure.}}
...
From cohort data, \textit{combined with a modeling framework that provides quantitative predictions based on alternative mortality regimes}, inferences can be made regarding plausible rates and mechanisms of mortality in the observed juvenile population (e.g. \cite{Schneider_2003}).
...
These models could give insight into \textit{why} these variant don't exist, in the context of implied changes in performance or distribution, or evolutionary costs and benefits.

\textit{\textbf{Models are tools for communicating biological knowledge across organizational levels.}}
...
\textit{\textbf{Models let you make scientific discoveries anytime, anywhere.}}
